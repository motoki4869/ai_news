...
- **変更**: 成功時は当日の日付を確認した`line_message.txt`をLINEへ送り、失敗時はSUMMARYまたはAPIエラーから原因を抽出して終了コードとログ先とともに通知する。フック送信を日次処理の最終判定後へ遅らせ、異なる失敗理由・更新ロック競合も通知対象にした。
- **理由**: 固定の完了文だけが送られて用語集更新を読めず、日次処理が失敗してもLINEで原因を把握できなかったため。同日再実行時のcommit前通知や前日本文の誤送信も防ぐため。
- **対象**: `scripts/lib/line_notification_dedupe.sh`、`scripts/daily_news.sh`、`.claude/hooks/line_notify.sh`、`.codex/hooks/line_notify.sh`
- **commit**: `HEAD`
