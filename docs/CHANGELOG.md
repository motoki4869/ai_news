...
### 日次Codexフォールバックのモデルを固定
- **変更**: Claude利用上限後に起動するCodex CLIで`gpt-6-sol`を明示し、`CODEX_FALLBACK_MODEL`で変更できるようにした。
- **理由**: ユーザー共通設定の`gpt-6-luna`がChatGPTアカウントで未対応のため、日次フォールバックがHTTP 400で停止したため。
