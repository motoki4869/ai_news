...
- **変更**: 成功時は従来の更新文・URL・リンクプレビューを保ち、当日の日付を確認した`line_message.txt`から新規用語だけを末尾に加える。本文が古い場合や当日ニュースが未commitの場合は失敗通知へ切り替える。失敗時はSUMMARYまたはAPIエラーから原因を抽出し、終了コードとログ先とともに通知する。フック送信を日次処理の最終判定後へ遅らせ、異なる失敗理由・更新ロック競合も通知対象にした。
- **理由**: 固定の完了文だけでは用語集更新を読めず、ニュース本文全体を通知すると従来の短い通知形式とリンクプレビューが失われるため。日次処理が失敗してもLINEで原因を把握できない問題と、同日再実行時のcommit前通知・前日本文の誤送信も防ぐため。
- **対象**: `scripts/lib/line_notification_dedupe.sh`、`scripts/daily_news.sh`、`.claude/hooks/line_notify.sh`、`.codex/hooks/line_notify.sh`、`test/`
- **commit**: `HEAD`

### 日次Codexフォールバックのモデルを動的選択
- **変更**: Codex app-serverからアカウントで利用可能なモデルを毎回取得し、API既定モデルを先頭に候補化する仕組みを追加した。任意の優先モデルは利用可能な場合だけ使い、モデル非対応エラー時は残りの候補へ切り替える。
- **理由**: 共通設定のモデルがChatGPTアカウントで未対応となり日次フォールバックが停止したため。特定モデル名に固定せず、将来のモデル変更にも追従させるため。
- **対象**: `scripts/lib/codex_fallback.sh`、`scripts/lib/resolve_codex_models.py`
- **commit**: `HEAD`

### 新規用語をLINE通知へ掲載
- **変更**: デイリーニュースから用語集へ追加した用語名と短い説明を、その日のLINE通知にも掲載するよう日次プロンプトを更新した。新規用語がない日は欄を省略し、同日再実行時は未commitの用語集差分から掲載対象を復元する。
- **理由**: ニュースを通じて新しく用語集に加わった用語を、毎朝の通知から把握できるようにするため。
- **対象**: `scripts/daily_news_prompt.txt`、`scripts/daily_news_prompt.codex.txt`
- **commit**: `HEAD`

## 2026-09-21

### デイリー最新ボタンの配置変更
- **変更**: 「最新」ボタンをカレンダー見出しから日付・件数の行へ移し、右端に配置した。
- **理由**: ニュース本文を読んでいるときにも、現在の日付の行から最新日へすぐ移動できるようにするため。
- **対象**: `history/daily.html`、`test/test_daily_navigation.js`
- **commit**: `HEAD`

### 要約欄のスワイプ日付切り替え修正
- **変更**: デイリーの見出し一覧リンク上でも横スワイプを検出し、前後の日付へ切り替えられるようにした。音声プレイヤーや操作ボタン上のジェスチャーは引き続き除外する。
- **理由**: 要約欄のリンク上ではスワイプ開始を無視していたため、記事要約の上で左右にスワイプしても日付が移動しなかったため。
- **対象**: `history/daily.html`、`history/daily-navigation.js`、`test/test_daily_navigation.js`
- **commit**: `HEAD`
