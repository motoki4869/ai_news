...
- **変更**: 成功時は先頭行の日付が当日と一致する`line_message.txt`だけをLINEへ送り、本文が古い場合や当日ニュースが未commitの場合は失敗通知へ切り替える。失敗時はSUMMARYまたはAPIエラーから原因を抽出し、終了コードとログ先とともに通知する。フック送信を日次処理の最終判定後へ遅らせ、異なる失敗理由・更新ロック競合も通知対象にした。
- **理由**: 固定の完了文だけが送られて用語集更新を読めず、日次処理が失敗してもLINEで原因を把握できなかったため。同日再実行時のcommit前通知や前日本文の誤送信も防ぐため。
