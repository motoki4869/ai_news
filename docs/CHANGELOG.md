...
- **変更**: 成功時は`line_message.txt`のニュースと新規用語をLINEへ送り、失敗時はSUMMARYまたはAPIエラーから原因を抽出して終了コードとログ先とともに通知する。送信claimの形式を更新し、旧固定文通知が当日分を抑止しないようにした。
- **理由**: 固定の完了文だけが送られて用語集更新を読めず、日次処理が失敗してもLINEで原因を把握できなかったため。
