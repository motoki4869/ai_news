...
- 実装中に既存データの不整合が1件見つかった。`202607.md` の2つ目の `## 2026-07-26`（165行目）は、コミット `1a4fa36`（作成日時 2026-07-27 07:12）が追加した7月27日朝のジョブの出力であり、日付見出しの誤記だった。`## 2026-07-27` に訂正済み。同種の誤記に今後気づけるよう、スクリプトはファイル内の日付重複を検出したら警告を出す。
...
- `## YYYY-MM-DD` 形式の行を日付キーとして認識する。同一ファイル内に同じ日付見出しが複数回現れた場合は、出現順を保ったまま1つの日付にマージしたうえで、stderr に警告を出す（処理は続行する）。ファイルをまたいで同じ日付が現れた場合はデータ異常としてエラー終了する。
- 対象ファイルが1件も見つからない場合はエラー終了する。無人ジョブでパスが崩れたときに、蓄積済みの `daily-data.js` を空で上書きしないため。
- 出力は同一ディレクトリの一時ファイルに書いてから `os.replace()` で置き換える。書き込み途中で落ちても壊れたファイルが残らないようにする。
...
1. `python3 scripts/generate_daily_data.py` を実行し、警告ゼロで正常終了することを確認する。出力された日数・項目数が、`everyday_news/*.md` を直接数えた一意な日付数・`- ` 行数と一致することを確認する（2026-08-02 時点の実データは 24日分・141項目。日次ジョブが走れば増える）。
