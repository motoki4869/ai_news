...
- `everyday_news/*.md` の形式は 2026-08-02 時点で 23日分・141項目すべてが `- **見出し**: 本文（[出典](URL)）。` に一致し、例外行は存在しない。なお `## ` 見出しは24行あるが、`202607.md` の `## 2026-07-26` が2回出現しているため一意な日付は23日分である（日次ジョブが同日2回走った結果であり、正当なデータ）。
...
- `## YYYY-MM-DD` 形式の行を日付キーとして認識する。同一ファイル内に同じ日付見出しが複数回現れた場合は、出現順を保ったまま1つの日付にマージする（日次ジョブが同じ日に2回走ることがあるため）。ファイルをまたいで同じ日付が現れた場合はデータ異常としてエラー終了する。
...
1. `python3 scripts/generate_daily_data.py` を実行し、警告ゼロで正常終了することを確認する。出力された日数・項目数が、`everyday_news/*.md` を直接数えた一意な日付数・`- ` 行数と一致することを確認する（2026-08-02 時点の実データは 23日分・141項目。日次ジョブが走れば増える）。
