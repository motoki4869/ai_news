...
- [x] `scripts/generate_audio_data.py` の期待仕様をテストで固定する。
...
- [x] `test/test_generate_audio_data.py` を追加し、unittestで上記を検証する。
- [x] テストを通す実装を追加する。CLIは、当日ソース抽出モードと音声一覧出力モードを持たせる。
- [x] `python3 -m unittest discover -s test -p 'test_*.py'` を実行する。
- [x] 変更を `feat: add audio metadata generation` としてコミットする。

## Task 2: NotebookLM音声生成スクリプトと設定例を追加する

- [x] `scripts/generate_notebooklm_audio.sh` を追加する。
...
- [x] `scripts/notebooklm_audio.env.example` と `.gitignore` の設定例を追加する。実設定は `scripts/notebooklm_audio.env` とし、git管理外にする。
- [x] `bash -n scripts/generate_notebooklm_audio.sh` と設定不足・CLI不足の失敗経路を検証する。
- [x] 変更を `feat: add NotebookLM audio generation` としてコミットする。

## Task 3: 毎朝の更新処理へ非ブロッキングで組み込む

- [x] `scripts/daily_news.sh` に音声生成呼び出しを追加する。
...
- [x] `README.md` に初回設定、毎朝の処理、失敗時の扱い、音声保存場所を追記する。
- [x] 既存テスト、shell構文、音声生成スクリプトのモック実行を確認する。
- [x] 変更を `feat: integrate daily NotebookLM audio update` としてコミットする。

## Task 4: `ai_news_daily` に音声プレイヤーを追加する

- [x] `history/daily.html` に `history/audio-data.js` の読み込みと音声プレイヤー領域を追加する。
- [x] 日付選択時に該当日の音声を表示し、存在しない日は自然な空状態を表示する。
- [x] 標準の `<audio controls preload='metadata'>` を使い、スマートフォンでも操作できるレイアウトにする。
- [x] `history/audio-data.js` を生成し、既存のページ表示とモバイルCSSを確認する。
- [x] 変更を `feat: add daily news audio player` としてコミットする。

## Task 5: 今日の音声を生成し、公開サイトへ反映する

- [x] NotebookLMに専用ノートブック「AIニュース音声」を作成し、ローカル設定へIDを保存する。
- [x] `2026-08-28` の記事セクションだけで音声を生成し、`history/audio/2026-08-28.m4a` を作成する。
- [x] 音声メタデータを生成し、ローカルHTTP配信でHTMLとM4Aの参照を確認する。
- [x] Vercel本番へpushし、最新ProductionデプロイがReadyであることを確認する。公開URLの匿名HTTPは既存のVercel SSO保護により302となるため、保護設定は変更していない。
- [x] push前に `git diff --check` と全テストを実行する。
- [x] 最終変更をコミットしてpushする。既存の設計書・計画書・実装・今日の音声メタデータを同じリポジトリに残す。
