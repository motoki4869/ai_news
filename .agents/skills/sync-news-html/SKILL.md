...
0. **更新ロックを取得する。** 変更を始める前に、リポジトリのルートで `source scripts/lib/news_update_lock.sh && acquire_news_update_lock "$PWD" --lease` を実行する。取得に失敗した場合は、日次更新または別の週次更新が実行中なので変更せず停止する。取得したトークンは `.news-update.lock/owner` に保存されるため、以降のBash呼び出しでは毎回 `LOCK_TOKEN="$(sed -n 's/^token=//p' .news-update.lock/owner)"` で読み直し、長い処理の前には `source scripts/lib/news_update_lock.sh && refresh_news_update_lock "$PWD" "$LOCK_TOKEN"` を実行する。別のBash呼び出しで環境変数やtrapを引き継げないため、シェル変数を手順間で持ち回らない。手順14のpush完了後に同じ方法でトークンを読み直し、`source scripts/lib/news_update_lock.sh && release_news_update_lock "$PWD" "$LOCK_TOKEN"` を実行する。leaseは一定時間更新されなければ回収される。日次処理と同じ原本・生成物・Git indexを触るため、このロックを省略しない。
