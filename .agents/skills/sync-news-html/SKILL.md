...
0. **更新ロックを取得する。** 変更を始める前に、リポジトリのルートで `source scripts/lib/news_update_lock.sh && acquire_news_update_lock "$PWD"` を実行する。取得に失敗した場合は、日次更新または別の週次更新が実行中なので変更せず停止する。取得後は `trap 'release_news_update_lock "$PWD"' EXIT` を設定し、手順14のpush完了までロックを保持する。日次処理と同じ原本・生成物・Git indexを触るため、このロックを省略しない。
