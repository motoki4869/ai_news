...
0. **更新ロックを取得する。** 変更を始める前に、リポジトリのルートで `LOCK_TOKEN="$(source scripts/lib/news_update_lock.sh && acquire_news_update_lock "$PWD" --lease)"` を実行する。取得に失敗した場合は、日次更新または別の週次更新が実行中なので変更せず停止する。トークンは手順14まで保持し、長い処理の前には `source scripts/lib/news_update_lock.sh && refresh_news_update_lock "$PWD" "$LOCK_TOKEN"` を実行する。別のBash呼び出しで環境変数やtrapを引き継げないため、取得時のプロセス終了だけで解放される方法は使わない。手順14のpush完了後に `source scripts/lib/news_update_lock.sh && release_news_update_lock "$PWD" "$LOCK_TOKEN"` を実行する。leaseは一定時間更新されなければ回収される。日次処理と同じ原本・生成物・Git indexを触るため、このロックを省略しない。
