...
0. **更新ロックを取得する。** 変更を始める前に、リポジトリのルートで `source scripts/lib/news_update_lock.sh && acquire_news_update_lock "$PWD" --lease` を実行する。取得に失敗した場合は、日次更新または別の週次更新が実行中なので変更せず停止する。コマンドの標準出力に表示されたトークン文字列をそのまま控え、以降のBash呼び出しではその**取得時のトークン文字列を直接** `source scripts/lib/news_update_lock.sh && refresh_news_update_lock "$PWD" "<取得時のトークン>"` と `source scripts/lib/news_update_lock.sh && release_news_update_lock "$PWD" "<取得時のトークン>"` に埋め込む。`.news-update.lock/owner`を読み直してトークンを得てはならない。別のBash呼び出しで環境変数やtrapを引き継げず、lease回収後に別ジョブが同じownerを作る可能性があるためである。手順7と手順11の直前を含む長い処理の前にrefreshし、refreshが非0を返した場合はロックを失っているため、以降の編集・git add・commit・pushを行わず停止して報告する。手順14のpush完了後にreleaseする。leaseは一定時間更新されなければ回収される。日次処理と同じ原本・生成物・Git indexを触るため、このロックを省略しない。
...
12. **用語集を最新化する。** 今回 news.html に追加した新規カードの本文を読み直し、`docs/glossary.md` に未収録で、かつ説明なしでは読み手が詰まる用語（新しい略語・規格名・モデル名・技術用語）があるかを確認する。
    - **収録基準**: サイト内に実際に登場する用語だけを入れる。一般的なAI辞書に寄せない。一度しか出てこない固有名詞や、文脈から自明な語は入れない。追加すべき語が無ければ何も足さずに次へ進む。
